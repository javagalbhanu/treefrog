...
public class utils {
	...
	public final static Path exlusionsFilepath = Paths.get(userHome
			+ "/exclusions.cfg");
	...
	public static <S extends Pane> Stage createDialogStage(String resource,
			Modality modality, Stage parent) {
	...
	private static File[] getLinuxVolumes() {
		...
		try {
			...
			while (scanner.hasNextLine()) {
				...
				// valid device. Look for the next forward slash following a
				// space
				...
				if (line.startsWith("/")) {
					...
					files.add(Paths.get(line.substring(startIdx, endIdx))
							.toFile());
