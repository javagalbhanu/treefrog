...
public class BufferedFileSystem extends FileSystem {
	...
	private final ExecutorService mWatchServiceExecutor = createExecutor(
			"WatchService", true);

	private final ListProperty<SyncPath> mBufferPaths = new SimpleListProperty<SyncPath>();
	...
	protected void construct() {
		...
		// model to allow executoin on the model's thread for list change
		// updates
		...
		mBufferPaths.addListener(new ListChangeListener<SyncPath>() {
			 ...
			 * 
			 * Changes on sync paths: publish to the outside. Changes on .cache
			 * paths: update the sync path.
			 */
			...
			public void onChanged(
				...
				// iterate the sync paths, moving cached files to their final
				// destination
	...
	private void commitFiles() {
		/*
		 * Executes a task to move a cached file from it's source to it's
		 * destination
		 */
		if (mFileCommits.isEmpty())
			return;
		...
		for (SyncPath p : mFileCommits) {
			...
			SyncPath cache_source = takeCachedFile(Integer.parseInt(p.getFile()
					.getName()));
			...
			moveFile(p.getPath(), target, StandardCopyOption.REPLACE_EXISTING,
	...
	private void publishFiles() {
		/*
		 * Add a syncpath to the changed paths property for application-wide
		 * notification of file changes (UI updates, binding updates, etc)
		 */
		if (mFilePublishes.isEmpty())
			return;
	...
	public synchronized boolean deleteFiles(List<SyncPath> paths) {
		...
		for (SyncPath p : paths) {
			...
			try {
				...
				success = success
						& Files.deleteIfExists(getRootPath().resolve(
								p.getRelativePath()));
			} catch (IOException e) {
