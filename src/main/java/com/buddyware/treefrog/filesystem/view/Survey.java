...
class ComplaintsPage extends WizardPage {
	...
	Parent getContent() {
		...
		return VBoxBuilder.create().spacing(5)
				.children(new Label("Do you have complaints?"), yes, no)
				.build();
	}
...
class MoreInformationPage extends WizardPage {
	...
	Parent getContent() {
		TextArea textArea = TextAreaBuilder.create().wrapText(true)
				.promptText("Tell me what's wrong Dave...").build();
		nextButton.setDisable(true);
		...
		return VBoxBuilder.create().spacing(5)
				.children(new Label("Please enter your complaints."), textArea)
				.build();
	}
}
...
class ThanksPage extends WizardPage {
	...
	Parent getContent() {
		StackPane stack = StackPaneBuilder.create()
				.children(new Label("Thanks!")).build();
