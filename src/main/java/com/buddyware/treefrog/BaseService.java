...
public class BaseService extends Service<Void> {
	...
	private final BlockingQueue<String> activityBlockingQueue = new ArrayBlockingQueue<>(
			1);
	private final BlockingQueue<String> errorBlockingQueue = new ArrayBlockingQueue<>(
			1);

	private final List<String> activityQueue = new ArrayList<String>();
	private final List<String> errorQueue = new ArrayList<String>();

	private final IntegerProperty errorFlag = new SimpleIntegerProperty(this,
			"int", 0);
	private final IntegerProperty activityFlag = new SimpleIntegerProperty(
			this, "int", 0);
	...
	public void putActivityMessage(String message) {
		...
		if (isBlocking) {
			System.out.println("Putting activity message " + message
					+ isBlocking);
			System.out.println("queue hash: "
					+ System.identityHashCode(activityBlockingQueue));
	...
	public IntegerProperty errorMessageFlag() {
		return errorFlag;
	};

	public IntegerProperty activityMessateFlag() {
		return activityFlag;
	};
